...
func testSendOutputs(r *Harness, t *testing.T) {
	genSpend := func(amt floutil.Amount) *chainhash.Hash {
		...
		// Next, send amt FLO to this address, spending from one of our mature
		// coinbase outputs.
	...
	// input.
	txid := genSpend(floutil.Amount(5 * floutil.SatoshiPerBitcoin))
	...
	txid = genSpend(floutil.Amount(500 * floutil.SatoshiPerBitcoin))
	blockHashes, err = r.Node.Generate(1)
	if err != nil {
		t.Fatalf("unable to generate single block: %v", err)
	}
...
func testGenerateAndSubmitBlock(r *Harness, t *testing.T) {
	...
	output := wire.NewTxOut(floutil.SatoshiPerBitcoin, pkScript)

	const numTxns = 5
	txns := make([]*floutil.Tx, 0, numTxns)
	for i := 0; i < numTxns; i++ {
		...
		txns = append(txns, floutil.NewTx(tx))
	}
...
func testMemWalletReorg(r *Harness, t *testing.T) {
	...
	// The internal wallet of this harness should now have 250 FLO.
	expectedBalance := floutil.Amount(250 * floutil.SatoshiPerBitcoin)
	...
	// The original wallet should now have a balance of 0 FLO as its entire
	// chain should have been decimated in favor of the main harness'
	// chain.
	expectedBalance = floutil.Amount(0)
	walletBalance = harness.ConfirmedBalance()
	if expectedBalance != walletBalance {
		t.Fatalf("wallet balance incorrect: expected %v, got %v",
			expectedBalance, walletBalance)
...
func testMemWalletLockedOutputs(r *Harness, t *testing.T) {
	...
	outputAmt := floutil.Amount(50 * floutil.SatoshiPerBitcoin)
	...
	// The current wallet balance should now be at least 50 FLO less
...
func TestHarness(t *testing.T) {
	// We should have (numMatureOutputs * 50 FLO) of mature unspendable
	// outputs.
	expectedBalance := floutil.Amount(numMatureOutputs * 50 * floutil.SatoshiPerBitcoin)
