...
func makeSpendableOutForTx(tx *wire.MsgTx, txOutIndex uint32) spendableOut {
	return spendableOut{
		...
		amount: floutil.Amount(tx.TxOut[txOutIndex].Value),
	}
}
...
func payToScriptHashScript(redeemScript []byte) []byte {
	redeemScriptHash := floutil.Hash160(redeemScript)
...
func calcMerkleRoot(txns []*wire.MsgTx) chainhash.Hash {
	...
	utilTxns := make([]*floutil.Tx, 0, len(txns))
	for _, tx := range txns {
		utilTxns = append(utilTxns, floutil.NewTx(tx))
	}
...
func additionalCoinbase(amount floutil.Amount) func(*wire.MsgBlock) {
...
func additionalSpendFee(fee floutil.Amount) func(*wire.MsgBlock) {
	return func(b *wire.MsgBlock) {
...
// tests.
func createSpendTx(spend *spendableOut, fee floutil.Amount) *wire.MsgTx {
...
func createSpendTxForTx(tx *wire.MsgTx, fee floutil.Amount) *wire.MsgTx {
