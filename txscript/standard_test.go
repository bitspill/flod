...
// newAddressPubKey returns a new floutil.AddressPubKey from the provided
...
func newAddressPubKey(serializedPubKey []byte) floutil.Address {
	addr, err := floutil.NewAddressPubKey(serializedPubKey,
...
// newAddressPubKeyHash returns a new floutil.AddressPubKeyHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressPubKeyHash(pkHash []byte) floutil.Address {
	addr, err := floutil.NewAddressPubKeyHash(pkHash, &chaincfg.MainNetParams)
	if err != nil {
...
// newAddressScriptHash returns a new floutil.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) floutil.Address {
	addr, err := floutil.NewAddressScriptHashFromHash(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
...
// bogusAddress implements the floutil.Address interface so the tests can ensure
...
// IsForNet lies blatantly to satisfy the floutil.Address interface.
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2pkhMain, err := floutil.NewAddressPubKeyHash(hexToBytes("e34cce70c86"+
	...
	p2shMain, _ := floutil.NewAddressScriptHashFromHash(hexToBytes("e8c300"+
	...
	p2pkCompressedMain, err := floutil.NewAddressPubKey(hexToBytes("02192d"+
	...
	p2pkCompressed2Main, err := floutil.NewAddressPubKey(hexToBytes("03b0b"+
	...
	p2pkUncompressedMain, err := floutil.NewAddressPubKey(hexToBytes("0411"+
	...
	tests := []struct {
		...
		{(*floutil.AddressPubKeyHash)(nil), "", errUnsupportedAddress},
		{(*floutil.AddressScriptHash)(nil), "", errUnsupportedAddress},
		{(*floutil.AddressPubKey)(nil), "", errUnsupportedAddress},
...
func TestMultiSigScript(t *testing.T) {
	...
	tests := []struct {
		keys      []*floutil.AddressPubKey
		nrequired int
		expected  string
		err       error
	}{
		{
			[]*floutil.AddressPubKey{
				p2pkCompressedMain,
				p2pkCompressed2Main,
			},
			1,
