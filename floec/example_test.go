...
func Example_signMessage() {
	...
	privKey, pubKey := floec.PrivKeyFromBytes(floec.S256(), pkBytes)
...
func Example_verifySignature() {
	...
	pubKey, err := floec.ParsePubKey(pubKeyBytes, floec.S256())
	if err != nil {
		fmt.Println(err)
		return
	}
...
func Example_encryptMessage() {
	...
	message := "test message"
	ciphertext, err := floec.Encrypt(pubKey, []byte(message))
	if err != nil {
		fmt.Println(err)
		return
	}
	...
	privKey, _ := floec.PrivKeyFromBytes(floec.S256(), pkBytes)

	// Try decrypting and verify if it's the same message.
	plaintext, err := floec.Decrypt(privKey, ciphertext)
	if err != nil {
		fmt.Println(err)
		return
	}
