...
const (
	...
	DefaultMinRelayTxFee = floutil.Amount(1000)
...
// pool and relayed.
func calcMinRequiredTxRelayFee(serializedSize int64, minRelayTxFee floutil.Amount) int64 {
	...
	if minFee < 0 || minFee > floutil.MaxSatoshi {
		minFee = floutil.MaxSatoshi
	}
...
func checkInputsStandard(tx *floutil.Tx, utxoView *blockchain.UtxoViewpoint) error {
...
func isDust(txOut *wire.TxOut, minRelayTxFee floutil.Amount) bool {
...
func checkTransactionStandard(tx *floutil.Tx, height int32,
	medianTimePast time.Time, minRelayTxFee floutil.Amount,
...
func GetTxVirtualSize(tx *floutil.Tx) int64 {
	// vSize := (weight(tx) + 3) / 4
