...
// oldFlodHomeDir returns the OS specific home directory flod used prior to
// version 0.3.3.  This has since been replaced with floutil.AppDataDir, but
// this function is still provided for the automatic upgrade path.
func oldFlodHomeDir() string {
	// Search for Windows APPDATA first.  This won't exist on POSIX OSes.
	appData := os.Getenv("APPDATA")
	if appData != "" {
		return filepath.Join(appData, "flod")
	}

	// Fall back to standard HOME directory that works for most POSIX OSes.
	home := os.Getenv("HOME")
	if home != "" {
		return filepath.Join(home, ".flod")
	}
...
// location prior to flod version 0.2.0 and uses heuristics to ascertain the old
...
// upgradeDBPaths moves the databases from their locations prior to flod
...
func upgradeDBPaths() error {
	...
	oldDbRoot := filepath.Join(oldFlodHomeDir(), "db")
	upgradeDBPathNet(filepath.Join(oldDbRoot, "flod.db"), "mainnet")
	upgradeDBPathNet(filepath.Join(oldDbRoot, "flod_testnet.db"), "testnet")
	upgradeDBPathNet(filepath.Join(oldDbRoot, "flod_regtest.db"), "regtest")

	// Remove the old db directory.
	return os.RemoveAll(oldDbRoot)
}

// upgradeDataPaths moves the application data from its location prior to flod
...
func upgradeDataPaths() error {
	...
	if fileExists(oldHomePath) && !fileExists(newHomePath) {
		// Create the new path.
		flodLog.Infof("Migrating application home path from '%s' to '%s'",
		...
		// Move old flod.conf into new location if needed.
		...
		if ohpEmpty {
			...
			flodLog.Warnf("Not removing '%s' since it contains files "+
...
// doUpgrades performs upgrades to flod as new versions require it.
