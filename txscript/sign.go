...
func RawTxInWitnessSignature(tx *wire.MsgTx, sigHashes *TxSigHashes, idx int,
	...
	key *floec.PrivateKey) ([]byte, error) {
...
// WitnessSignature creates an input witness stack for tx to spend FLO sent
...
func WitnessSignature(tx *wire.MsgTx, sigHashes *TxSigHashes, idx int, amt int64,
	subscript []byte, hashType SigHashType, privKey *floec.PrivateKey,
	...
	pk := (*floec.PublicKey)(&privKey.PublicKey)
	var pkData []byte
	if compress {
...
func RawTxInSignature(tx *wire.MsgTx, idx int, subScript []byte,
	hashType SigHashType, key *floec.PrivateKey) ([]byte, error) {
...
// SignatureScript creates an input signature script for tx to spend FLO sent
...
func SignatureScript(tx *wire.MsgTx, idx int, subscript []byte, hashType SigHashType, privKey *floec.PrivateKey, compress bool) ([]byte, error) {
...
func p2pkSignatureScript(tx *wire.MsgTx, idx int, subScript []byte, hashType SigHashType, privKey *floec.PrivateKey) ([]byte, error) {
	sig, err := RawTxInSignature(tx, idx, subScript, hashType, privKey)
	if err != nil {
		return nil, err
	}
...
func signMultiSig(tx *wire.MsgTx, idx int, subScript []byte, hashType SigHashType,
	addresses []floutil.Address, nRequired int, kdb KeyDB) ([]byte, bool) {
...
func sign(chainParams *chaincfg.Params, tx *wire.MsgTx, idx int,
	...
	ScriptClass, []floutil.Address, int, error) {
...
func mergeScripts(chainParams *chaincfg.Params, tx *wire.MsgTx, idx int,
	pkScript []byte, class ScriptClass, addresses []floutil.Address,
...
func mergeMultiSig(tx *wire.MsgTx, idx int, addresses []floutil.Address,
...
sigLoop:
	for _, sig := range possibleSigs {
		...
		pSig, err := floec.ParseDERSignature(tSig, floec.S256())
		if err != nil {
			continue
		}
		...
		for _, addr := range addresses {
			...
			// bad input.
			pkaddr := addr.(*floutil.AddressPubKey)
...
type KeyDB interface {
	GetKey(floutil.Address) (*floec.PrivateKey, bool, error)
}

// KeyClosure implements KeyDB with a closure.
type KeyClosure func(floutil.Address) (*floec.PrivateKey, bool, error)

// GetKey implements KeyDB by returning the result of calling the closure.
func (kc KeyClosure) GetKey(address floutil.Address) (*floec.PrivateKey,
	bool, error) {
	return kc(address)
}
...
type ScriptDB interface {
	GetScript(floutil.Address) ([]byte, error)
}

// ScriptClosure implements ScriptDB with a closure.
type ScriptClosure func(floutil.Address) ([]byte, error)

// GetScript implements ScriptDB by returning the result of calling the closure.
func (sc ScriptClosure) GetScript(address floutil.Address) ([]byte, error) {
	return sc(address)
}
