...
func TestHaveBlock(t *testing.T) {
	...
	_, isOrphan, err := chain.ProcessBlock(floutil.NewBlock(&Block100000),
		BFNone)
	if err != nil {
...
func TestCalcSequenceLock(t *testing.T) {
	...
	// age of 4 blocks.
	targetTx := floutil.NewTx(&wire.MsgTx{
		TxOut: []*wire.TxOut{{
			PkScript: nil,
			Value:    10,
		}},
	...
	utxoView.AddTxOuts(floutil.NewTx(unConfTx), 0x7fffffff)
