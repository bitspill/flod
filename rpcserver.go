...
var (
	...
	// data.
	gbtCoinbaseAux = &flojson.GetBlockTemplateResultAux{
...
var (
	...
	ErrRPCUnimplemented = &flojson.RPCError{
		Code:    flojson.ErrRPCUnimplemented,
		Message: "Command unimplemented",
	}

	// ErrRPCNoWallet is an error returned to RPC clients when the provided
	// command is recognized as a wallet command.
	ErrRPCNoWallet = &flojson.RPCError{
		Code:    flojson.ErrRPCNoWallet,
...
// list of commands that we recognize, but for which flod has no support because
// it lacks support for wallet functionality. For these commands the user
// should ask a connected instance of flowallet.
...
// not needed.
func internalRPCError(errStr, context string) *flojson.RPCError {
	logStr := errStr
	if context != "" {
		logStr = context + ": " + errStr
	}
	rpcsLog.Error(logStr)
	return flojson.NewRPCError(flojson.ErrRPCInternal.Code, errStr)
}

// rpcDecodeHexError is a convenience function for returning a nicely formatted
// RPC error which indicates the provided hex string failed to decode.
func rpcDecodeHexError(gotHex string) *flojson.RPCError {
	return flojson.NewRPCError(flojson.ErrRPCDecodeHexString,
...
// transaction hash.
func rpcNoTxInfoError(txHash *chainhash.Hash) *flojson.RPCError {
	return flojson.NewRPCError(flojson.ErrRPCNoTxInfo,
...
// These commands will be implemented in flowallet.
...
func messageToHex(msg wire.Message) (string, error) {
	...
	if err := msg.FloEncode(&buf, maxProtocolVersion, wire.WitnessEncoding); err != nil {
...
func handleCreateRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*flojson.CreateRawTransactionCmd)
	...
	for encodedAddr, amount := range c.Amounts {
		...
		if amount <= 0 || amount > floutil.MaxSatoshi {
			return nil, &flojson.RPCError{
		...
		addr, err := floutil.DecodeAddress(encodedAddr, params)
		if err != nil {
			return nil, &flojson.RPCError{
		...
		satoshi, err := floutil.NewAmount(amount)
		if err != nil {
...
// transaction.
func createVinList(mtx *wire.MsgTx) []flojson.Vin {
	// Coinbase transactions only have a single txin by definition.
	vinList := make([]flojson.Vin, len(mtx.TxIn))
	...
	for i, txIn := range mtx.TxIn {
		...
		vinEntry.ScriptSig = &flojson.ScriptSig{
			Asm: disbuf,
...
// transaction.
func createVoutList(mtx *wire.MsgTx, chainParams *chaincfg.Params, filterAddrMap map[string]struct{}) []flojson.Vout {
	voutList := make([]flojson.Vout, 0, len(mtx.TxOut))
	for i, v := range mtx.TxOut {
		...
		var vout flojson.Vout
		vout.N = uint32(i)
		vout.Value = floutil.Amount(v.Value).ToFLO()
...
func createTxRawResult(chainParams *chaincfg.Params, mtx *wire.MsgTx,
	...
	blkHeight int32, chainHeight int32) (*flojson.TxRawResult, error) {
	...
	txReply := &flojson.TxRawResult{
		Hex:      mtxHex,
		Txid:     txHash,
		Hash:     mtx.WitnessHash().String(),
		Size:     int32(mtx.SerializeSize()),
		Vsize:    int32(mempool.GetTxVirtualSize(floutil.NewTx(mtx))),
...
func handleDecodeRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*flojson.DecodeRawTransactionCmd)
	...
	if err != nil {
		return nil, &flojson.RPCError{
			Code:    flojson.ErrRPCDeserialization,
			Message: "TX decode failed: " + err.Error(),
		}
	}

	// Create and return the result.
	txReply := flojson.TxRawDecodeResult{
...
func handleDecodeScript(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	p2sh, err := floutil.NewAddressScriptHash(script, s.cfg.ChainParams)
	if err != nil {
	...
	reply := flojson.DecodeScriptResult{
		Asm:       disbuf,
...
func handleGenerate(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	if len(cfg.miningAddrs) == 0 {
		return nil, &flojson.RPCError{
			Code: flojson.ErrRPCInternal.Code,
...
func handleGetAddedNodeInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*flojson.GetAddedNodeInfoCmd)
	...
	if c.Node != nil {
		...
		if !found {
			return nil, &flojson.RPCError{
				Code:    flojson.ErrRPCClientNodeNotAdded,
	...
	results := make([]*flojson.GetAddedNodeInfoResult, 0, len(peers))
	for _, rpcPeer := range peers {
		// Set the "address" of the peer which could be an ip address
		// or a domain name.
		peer := rpcPeer.ToPeer()
		var result flojson.GetAddedNodeInfoResult
		result.AddedNode = peer.Addr()
		result.Connected = flojson.Bool(peer.Connected())
		...
		addrs := make([]flojson.GetAddedNodeInfoResultAddr, 0, len(ipList))
		for _, ip := range ipList {
			var addr flojson.GetAddedNodeInfoResultAddr
			addr.Address = ip
...
func handleGetBestBlock(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	result := &flojson.GetBestBlockResult{
...
func handleGetBlock(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	blk, err := floutil.NewBlockFromBytes(blkBytes)
	if err != nil {
	...
	blockReply := flojson.GetBlockVerboseResult{
		Hash:          c.Hash,
	...
	if c.VerboseTx == nil || !*c.VerboseTx {
		...
		rawTxns := make([]flojson.TxRawResult, len(txns))
		for i, tx := range txns {
...
func handleGetBlockChainInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	chainInfo := &flojson.GetBlockChainInfoResult{
		Chain:         params.Name,
		...
		Pruned:        false,
		Bip9SoftForks: make(map[string]*flojson.Bip9SoftForkDescription),
	}
	...
	chainInfo.SoftForks = []*flojson.SoftForkDescription{
		{
			ID:      "bip34",
			Version: 2,
			Reject: struct {
	...
	for deployment, deploymentDetails := range params.Deployments {
		...
		chainInfo.Bip9SoftForks[forkName] = &flojson.Bip9SoftForkDescription{
...
func handleGetBlockHash(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*flojson.GetBlockHashCmd)
	hash, err := s.cfg.Chain.BlockHashByHeight(int32(c.Index))
	if err != nil {
		return nil, &flojson.RPCError{
			Code:    flojson.ErrRPCOutOfRange,
...
func handleGetBlockHeader(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*flojson.GetBlockHeaderCmd)
	...
	params := s.cfg.ChainParams
	blockHeaderReply := flojson.GetBlockHeaderVerboseResult{
		Hash:          c.Hash,
		Confirmations: uint64(1 + best.Height - blockHeight),
		Height:        blockHeight,
		Version:       blockHeader.Version,
...
// state as a flojson.GetBlockTemplateResult that is ready to be encoded to JSON
// and returned to the caller.
//
// This function MUST be called with the state locked.
func (state *gbtWorkState) blockTemplateResult(useCoinbaseValue bool, submitOld *bool) (*flojson.GetBlockTemplateResult, error) {
	...
	transactions := make([]flojson.GetBlockTemplateResultTx, 0, numTx-1)
	...
	for i, tx := range msgBlock.Transactions {
		...
		bTx := floutil.NewTx(tx)
		resultTx := flojson.GetBlockTemplateResultTx{
	...
	reply := flojson.GetBlockTemplateResult{
...
func handleGetBlockTemplateRequest(s *rpcServer, request *flojson.TemplateRequest, closeChan <-chan struct{}) (interface{}, error) {
	...
	if !(cfg.RegressionTest || cfg.SimNet) &&
		...
		return nil, &flojson.RPCError{
			Code:    flojson.ErrRPCClientNotConnected,
	...
	if currentHeight != 0 && !s.cfg.SyncMgr.IsCurrent() {
		return nil, &flojson.RPCError{
			Code:    flojson.ErrRPCClientInInitialDownload,
...
// chainErrToGBTErrString converts an error returned from flochain to a string
...
func handleGetBlockTemplateProposal(s *rpcServer, request *flojson.TemplateRequest) (interface{}, error) {
...
func handleGetBlockTemplate(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*flojson.GetBlockTemplateCmd)
	request := c.Request
...
func handleGetInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	ret := &flojson.InfoChainResult{
		...
		RelayFee:        cfg.minRelayTxFee.ToFLO(),
	}
...
func handleGetMempoolInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	ret := &flojson.GetMempoolInfoResult{
...
func handleGetMiningInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	gnhpsCmd := flojson.NewGetNetworkHashPSCmd(nil, nil)
	...
	best := s.cfg.Chain.BestSnapshot()
	result := flojson.GetMiningInfoResult{
...
func handleGetNetTotals(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	reply := &flojson.GetNetTotalsResult{
...
func handleGetNetworkHashPS(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*flojson.GetNetworkHashPSCmd)
...
func handleGetPeerInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	infos := make([]*flojson.GetPeerInfoResult, 0, len(peers))
	for _, p := range peers {
		statsSnap := p.ToPeer().StatsSnapshot()
		info := &flojson.GetPeerInfoResult{
			ID:             statsSnap.ID,
			Addr:           statsSnap.Addr,
...
func handleGetRawMempool(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*flojson.GetRawMempoolCmd)
	mp := s.cfg.TxMemPool
...
func handleGetRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*flojson.GetRawTransactionCmd)
...
func handleGetTxOut(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	if includeMempool && s.cfg.TxMemPool.HaveTransaction(txHash) {
		...
		if c.Vout > uint32(len(mtx.TxOut)-1) {
			return nil, &flojson.RPCError{
				Code: flojson.ErrRPCInvalidTxVout,
	...
	txOutReply := &flojson.GetTxOutResult{
		BestBlock:     bestBlockHash,
		Confirmations: int64(confirmations),
		Value:         floutil.Amount(value).ToFLO(),
		Version:       txVersion,
		ScriptPubKey: flojson.ScriptPubKeyResult{
			Asm:       disbuf,
...
// passed transaction.
func createVinListPrevOut(s *rpcServer, mtx *wire.MsgTx, chainParams *chaincfg.Params, vinExtra bool, filterAddrMap map[string]struct{}) ([]flojson.VinPrevOut, error) {
	// Coinbase transactions only have a single txin by definition.
	if blockchain.IsCoinBaseTx(mtx) {
		...
		txIn := mtx.TxIn[0]
		vinList := make([]flojson.VinPrevOut, 1)
		vinList[0].Coinbase = hex.EncodeToString(txIn.SignatureScript)
		vinList[0].Sequence = txIn.Sequence
		return vinList, nil
	}
	...
	vinList := make([]flojson.VinPrevOut, 0, len(mtx.TxIn))
	...
	for _, txIn := range mtx.TxIn {
		...
		if vinExtra {
			...
			vinListEntry.PrevOut = &flojson.PrevOut{
				Addresses: encodedAddrs,
				Value:     floutil.Amount(originTxOut.Value).ToFLO(),
			}
		}
	}
...
func fetchMempoolTxnsForAddress(s *rpcServer, addr floutil.Address, numToSkip, numRequested uint32) ([]*floutil.Tx, uint32) {
...
func handleSearchRawTransactions(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*flojson.SearchRawTransactionsCmd)
	vinExtra := false
	if c.VinExtra != nil {
	...
	params := s.cfg.ChainParams
	addr, err := floutil.DecodeAddress(c.Address, params)
	if err != nil {
		return nil, &flojson.RPCError{
			Code:    flojson.ErrRPCInvalidAddressOrKey,
			Message: "Invalid address or key: " + err.Error(),
	...
	// The verbose flag is set, so generate the JSON object and return it.
	best := s.cfg.Chain.BestSnapshot()
	srtList := make([]flojson.SearchRawTransactionsResult, len(addressTxns))
	for i := range addressTxns {
...
func handleSendRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*flojson.SendRawTransactionCmd)
...
func handleValidateAddress(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*flojson.ValidateAddressCmd)

	result := flojson.ValidateAddressChainResult{}
	addr, err := floutil.DecodeAddress(c.Address, s.cfg.ChainParams)
	if err != nil {
...
func handleVerifyMessage(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*flojson.VerifyMessageCmd)
	...
	address, err := floutil.NewAddressPubKey(serializedPK, params)
	if err != nil {
...
func handleVersion(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	result := map[string]flojson.VersionResult{
		"flodjsonrpcapi": {
...
func (s *rpcServer) standardCmdResult(cmd *parsedRPCCmd, closeChan <-chan struct{}) (interface{}, error) {
	...
	return nil, flojson.ErrRPCMethodNotFound
handled:
...
func parseCmd(request *flojson.Request) *parsedRPCCmd {
	...
	if err != nil {
		...
		if jerr, ok := err.(flojson.Error); ok &&
			jerr.ErrorCode == flojson.ErrUnregisteredMethod {

			parsedCmd.err = flojson.ErrRPCMethodNotFound
			return &parsedCmd
		}

		// Otherwise, some type of invalid parameters is the
		// cause, so produce the equivalent RPC error.
		parsedCmd.err = flojson.NewRPCError(
			flojson.ErrRPCInvalidParams.Code, err.Error())
		return &parsedCmd
	}
...
// the type *flojson.RPCError to the appropriate type as needed.
func createMarshalledReply(id, result interface{}, replyErr error) ([]byte, error) {
	var jsonErr *flojson.RPCError
	if replyErr != nil {
		if jErr, ok := replyErr.(*flojson.RPCError); ok {
			jsonErr = jErr
		} else {
	...
	return flojson.MarshalResponse(id, result, jsonErr)
}
...
func jsonAuthFail(w http.ResponseWriter) {
	w.Header().Add("WWW-Authenticate", `Basic realm="flod RPC"`)
...
type rpcserverSyncManager interface {
	...
	SubmitBlock(block *floutil.Block, flags blockchain.BehaviorFlags) (bool, error)
