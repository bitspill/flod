...
const (
	...
	unusableFlags = flojson.UFWebsocketOnly | flojson.UFNotification
)

var (
	flodHomeDir           = floutil.AppDataDir("flod", false)
	floctlHomeDir         = floutil.AppDataDir("floctl", false)
	flowalletHomeDir      = floutil.AppDataDir("flowallet", false)
	defaultConfigFile     = filepath.Join(floctlHomeDir, "floctl.conf")
	defaultRPCServer      = "localhost"
	defaultRPCCertFile    = filepath.Join(flodHomeDir, "rpc.cert")
	defaultWalletCertFile = filepath.Join(flowalletHomeDir, "rpc.cert")
)
...
func listCommands() {
	...
	cmdMethods := flojson.RegisteredCmdMethods()
	categorized := make([][]string, numCategories)
	for _, method := range cmdMethods {
		flags, err := flojson.MethodUsageFlags(method)
		if err != nil {
		...
		usage, err := flojson.MethodUsageText(method)
		if err != nil {
			// This should never happen since the method was just
			// returned from the package, but be safe.
			continue
		...
		if flags&flojson.UFWalletOnly != 0 {
...
// config defines the configuration options for floctl.
//
...
func cleanAndExpandPath(path string) string {
	...
	if strings.HasPrefix(path, "~") {
		homeDir := filepath.Dir(floctlHomeDir)
...
func loadConfig() (*config, []string, error) {
	...
	if _, err := os.Stat(preCfg.ConfigFile); os.IsNotExist(err) {
		// Use config file for RPC server to create default floctl config
		var serverConfigPath string
		if preCfg.Wallet {
			serverConfigPath = filepath.Join(flowalletHomeDir, "flowallet.conf")
		} else {
			serverConfigPath = filepath.Join(flodHomeDir, "flod.conf")
		}
...
// For this it tries to read the config file for the RPC server (either flod or
// flowallet), and extract the RPC user and password from it.
