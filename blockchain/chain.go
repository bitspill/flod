...
// exceeded.
func (b *BlockChain) addOrphanBlock(block *floutil.Block) {
...
//
// This function is safe for concurrent access.
func (b *BlockChain) CalcSequenceLock(tx *floutil.Tx, utxoView *UtxoViewpoint, mempool bool) (*SequenceLock, error) {
	b.chainLock.Lock()
...
func (b *BlockChain) calcSequenceLock(node *blockNode, tx *floutil.Tx, utxoView *UtxoViewpoint, mempool bool) (*SequenceLock, error) {
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) connectBlock(node *blockNode, block *floutil.Block, view *UtxoViewpoint, stxos []spentTxOut) error {
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) disconnectBlock(node *blockNode, block *floutil.Block, view *UtxoViewpoint) error {
...
func countSpentOutputs(block *floutil.Block) int {
...
func (b *BlockChain) reorganizeChain(detachNodes, attachNodes *list.List) error {
	...
	detachBlocks := make([]*floutil.Block, 0, detachNodes.Len())
	detachSpentTxOuts := make([][]spentTxOut, 0, detachNodes.Len())
	attachBlocks := make([]*floutil.Block, 0, attachNodes.Len())
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) connectBestChain(node *blockNode, block *floutil.Block, flags BehaviorFlags) (bool, error) {
...
type IndexManager interface {
	...
	// main chain.
	ConnectBlock(database.Tx, *floutil.Block, *UtxoViewpoint) error

	// DisconnectBlock is invoked when a block has been disconnected from
	// the main chain.
	DisconnectBlock(database.Tx, *floutil.Block, *UtxoViewpoint) error
}
