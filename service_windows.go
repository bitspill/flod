...
const (
	// svcName is the name of flod service.
	svcName = "flodsvc"
...
// logServiceStartOfDay logs information about flod when the main server has
...
// flodService houses the main service handler which handles all service
// updates and launching flodMain.
type flodService struct{}
...
// long-running flodMain (which is the real meat of flod), handles service
// change requests, and notifies the service control manager of changes.
func (s *flodService) Execute(args []string, r <-chan svc.ChangeRequest, changes chan<- svc.Status) (bool, uint32) {
	// Service start is pending.
	const cmdsAccepted = svc.AcceptStop | svc.AcceptShutdown
	changes <- svc.Status{State: svc.StartPending}

	// Start flodMain in a separate goroutine so the service can start
...
// installService attempts to install the flod service.  Typically this should
...
// removeService attempts to uninstall the flod service.  Typically this should
...
// startService attempts to start the flod service.
...
func serviceMain() (bool, error) {
	...
	err = svc.Run(svcName, &flodService{})
	if err != nil {
