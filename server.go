...
import (
	...
	"github.com/bitspill/flod/mining/cpuminer"
	"github.com/bitspill/flod/netsync"
	"github.com/bitspill/flod/peer"
	"github.com/bitspill/flod/txscript"
	"github.com/bitspill/flod/wire"
	"github.com/bitspill/floutil"
	"github.com/bitspill/floutil/bloom"
)
...
func (sp *serverPeer) OnTx(_ *peer.Peer, msg *wire.MsgTx) {
	...
	// Convert the raw MsgTx to a floutil.Tx which provides some convenience
	// methods and things such as hash caching.
	tx := floutil.NewTx(msg)
...
func (sp *serverPeer) OnBlock(_ *peer.Peer, msg *wire.MsgBlock, buf []byte) {
	// Convert the raw MsgBlock to a floutil.Block which provides some
	// convenience methods and things such as hash caching.
	block := floutil.NewBlockFromBlockAndBytes(msg, buf)
...
func (sp *serverPeer) OnFeeFilter(_ *peer.Peer, msg *wire.MsgFeeFilter) {
	...
	if msg.MinFee < 0 || msg.MinFee > floutil.MaxSatoshi {
		peerLog.Debugf("Peer %v sent an invalid feefilter '%v' -- "+
			"disconnecting", sp, floutil.Amount(msg.MinFee))
		sp.Disconnect()
		return
	}
...
func (s *server) TransactionConfirmed(tx *floutil.Tx) {
...
func (s *server) peerHandler() {
	...
	if !cfg.DisableDNSSeed {
		...
		connmgr.SeedFromDNS(activeNetParams.Params, defaultRequiredServices,
			flodLookup, func(addrs []*wire.NetAddress) {
...
// newServer returns a new flod server configured to listen on addr for the
...
func newServer(listenAddrs []string, db database.DB, chainParams *chaincfg.Params, interrupt <-chan struct{}) (*server, error) {
	...
	amgr := addrmgr.New(cfg.DataDir, flodLookup)
	...
	txC := mempool.Config{
		...
		CalcSequenceLock: func(tx *floutil.Tx, view *blockchain.UtxoViewpoint) (*blockchain.SequenceLock, error) {
