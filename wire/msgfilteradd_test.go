...
func TestFilterAddLatest(t *testing.T) {
	...
	err := msg.FloEncode(&buf, pver, enc)
	if err != nil {
	...
	err = readmsg.Flodecode(&buf, pver, enc)
	if err != nil {
...
func TestFilterAddCrossProtocol(t *testing.T) {
	...
	var buf bytes.Buffer
	err := msg.FloEncode(&buf, ProtocolVersion, LatestEncoding)
	if err != nil {
		t.Errorf("encode of MsgFilterAdd failed %v err <%v>", msg, err)
	}
	...
	var readmsg MsgFilterAdd
	err = readmsg.Flodecode(&buf, BIP0031Version, LatestEncoding)
	if err == nil {
...
func TestFilterAddMaxDataSize(t *testing.T) {
	...
	err = msg.Flodecode(readbuf, ProtocolVersion, LatestEncoding)
	if err == nil {
		t.Errorf("decode of MsgFilterAdd succeeded when it shouldn't "+
			"have %v", msg)
	}
