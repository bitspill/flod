...
import (
	...
	rpc "github.com/bitspill/flod/rpcclient"
	"github.com/bitspill/floutil"
)

// nodeConfig contains all the args, and data required to launch a flod process
...
func newConfig(prefix, certFile, keyFile string, extra []string) (*nodeConfig, error) {
	flodPath, err := flodExecutablePath()
	if err != nil {
		return nil, err
	}
...
// arguments returns an array of arguments that be used to launch the flod
// process.
...
// command returns the exec.Cmd which will be used to start the flod process.
...
// to the flod process that is launched via Start().
...
// as the base for the log and data directories for flod.
...
// start creates a new flod process, and writes its pid in a file reserved for
...
// stop interrupts the running flod process process, and waits until it exits
...
// shutdown terminates the running flod process, and cleans up all
...
func genCertPair(certFile, keyFile string) error {
	...
	cert, key, err := floutil.NewTLSCertPair(org, validUntil, nil)
	if err != nil {
		return err
	}
