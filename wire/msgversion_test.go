...
func TestVersionWireErrors(t *testing.T) {
	...
	// Ensure calling MsgVersion.Flodecode with a non *bytes.Buffer returns
	// error.
	fr := newFixedReader(0, []byte{})
	if err := baseVersion.Flodecode(fr, pver, enc); err == nil {
		t.Errorf("Did not received error when calling " +
			"MsgVersion.Flodecode with non *bytes.Buffer")
	}
	...
	for i, test := range tests {
		...
		err = msg.Flodecode(buf, test.pver, test.enc)
...
func TestVersionOptionalFields(t *testing.T) {
	...
	uaVersion.UserAgent = "/flodtest:0.0.1/"
...
var baseVersionEncoded = []byte{
	...
	0x2f, 0x66, 0x6c, 0x6f, 0x64, 0x74, 0x65, 0x73,
