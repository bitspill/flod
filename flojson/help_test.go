...
func TestHelpReflectInternals(t *testing.T) {
	...
	for i, test := range tests {
		...
		key := flojson.TstReflectTypeToJSONType(xT, test.reflectType)
		if key != test.key {
		...
		examples, isComplex := flojson.TstReflectTypeToJSONExample(xT,
		...
		helpText := flojson.TstResultTypeHelp(xT, test.reflectType, "fdk")
		...
		isValid := flojson.TstIsValidResultType(test.reflectType.Kind())
...
func TestResultStructHelp(t *testing.T) {
	...
	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
		results := flojson.TstResultStructHelp(xT, test.reflectType, 0)
...
func TestHelpArgInternals(t *testing.T) {
	...
	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
		help := flojson.TstArgHelp(xT, test.reflectType, test.defaults,
			test.method)
		if help != test.help {
...
func TestMethodHelp(t *testing.T) {
	...
	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
		help := flojson.TestMethodHelp(xT, test.reflectType,
...
func TestGenerateHelpErrors(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			method: "boguscommand",
			err:    flojson.Error{ErrorCode: flojson.ErrUnregisteredMethod},
		},
		{
		...
		},
		{
			...
			err:         flojson.Error{ErrorCode: flojson.ErrMissingDescription},
		},
	}

	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
		_, err := flojson.GenerateHelp(test.method, nil,
			test.resultTypes...)
...
func TestGenerateHelp(t *testing.T) {
	...
	help, err := flojson.GenerateHelp("help", descs)
	if err != nil {
