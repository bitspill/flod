...
type memWallet struct {
	...
	addrs map[uint32]floutil.Address
...
func newMemWallet(net *chaincfg.Params, harnessID uint32) (*memWallet, error) {
	...
	addrs := make(map[uint32]floutil.Address)
...
// SetRPCClient saves the passed rpc connection to flod as the wallet's
...
func (m *memWallet) IngestBlock(height int32, header *wire.BlockHeader, filteredTxns []*floutil.Tx) {
...
func (m *memWallet) evalOutputs(outputs []*wire.TxOut, txHash *chainhash.Hash,
	...
	for i, output := range outputs {
		...
		for keyIndex, addr := range m.addrs {
			...
			m.utxos[op] = &utxo{
				value:          floutil.Amount(output.Value),
				keyIndex:       keyIndex,
...
func (m *memWallet) newAddress() (floutil.Address, error) {
	index := m.hdIndex
	...
	err = m.rpc.LoadTxFilter(false, []floutil.Address{addr}, nil)
	if err != nil {
		return nil, err
	}
...
func (m *memWallet) fundTx(tx *wire.MsgTx, amt floutil.Amount, feeRate floutil.Amount) error {
	const (
	...
	for outPoint, utxo := range m.utxos {
		...
		reqFee := floutil.Amount(txSize * int(feeRate))
...
//
// This function is safe for concurrent access.
func (m *memWallet) CreateTransaction(outputs []*wire.TxOut, feeRate floutil.Amount) (*wire.MsgTx, error) {
	m.Lock()
	defer m.Unlock()
	...
	var outputAmt floutil.Amount
	for _, output := range outputs {
		outputAmt += floutil.Amount(output.Value)
		tx.AddTxOut(output)
	}
...
//
// This function is safe for concurrent access.
func (m *memWallet) ConfirmedBalance() floutil.Amount {
	m.RLock()
	defer m.RUnlock()

	var balance floutil.Amount
