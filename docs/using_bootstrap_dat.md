...
5. [How do I use bootstrap.dat with flod?](#Importing)
...
**NOTE:** Using bootstrap.dat is entirely optional.  Flod will download the
...
### 5. How do I use bootstrap.dat with flod?

flod comes with a separate utility named `addblock` which can be used to import
...
1. Stop flod if it is already running.  This is required since addblock needs to
   access the database used by flod and it will be locked if flod is using it.
...
**Windows:**
```bat
C:\> "%PROGRAMFILES%\Flod Suite\Flod\addblock" -i C:\Path\To\bootstrap.dat
```
