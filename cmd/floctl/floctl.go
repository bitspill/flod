...
func main() {
	...
	method := args[0]
	usageFlags, err := flojson.MethodUsageFlags(method)
	if err != nil {
	...
	cmd, err := flojson.NewCmd(method, params...)
	if err != nil {
		// Show the error along with its error code when it's a
		// flojson.Error as it reallistcally will always be since the
		// NewCmd function is only supposed to return errors of that
		// type.
		if jerr, ok := err.(flojson.Error); ok {
		...
		// The error is not a flojson.Error and this really should not
	...
	marshalledJSON, err := flojson.MarshalCmd(1, cmd)
	if err != nil {
