...
func TestIsValidIDType(t *testing.T) {
	...
	for i, test := range tests {
		if flojson.IsValidIDType(test.id) != test.isValid {
...
func TestMarshalResponse(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			result: nil,
			jsonErr: func() *flojson.RPCError {
				return flojson.NewRPCError(flojson.ErrRPCBlockNotFound, "123 not found")
			}(),
	...
	for i, test := range tests {
		_, _ = i, test
		marshalled, err := flojson.MarshalResponse(testID, test.result, test.jsonErr)
		if err != nil {
...
func TestMiscErrors(t *testing.T) {
	...
	// not supported.
	_, err := flojson.NewRequest(nil, "test", []interface{}{make(chan int)})
	if err == nil {
	...
	// supported.
	wantErr := flojson.Error{ErrorCode: flojson.ErrInvalidType}
	_, err = flojson.MarshalResponse(make(chan int), nil, nil)
	if jerr, ok := err.(flojson.Error); !ok || jerr.ErrorCode != wantErr.ErrorCode {
	...
	// can't be marshalled.
	_, err = flojson.MarshalResponse(1, make(chan int), nil)
...
func TestRPCError(t *testing.T) {
	...
	tests := []struct {
		...
		{
			flojson.ErrRPCInvalidRequest,
			"-32600: Invalid request",
		},
		{
			flojson.ErrRPCMethodNotFound,
