...
flod provides full support for anonymous networking via the
...
[stream isolation](#TorStreamIsolation).  In addition, flod supports a hybrid,
...
Configuring flod as a Tor client is straightforward.  The first step is
obviously to install Tor and ensure it is working. Once that is done, all that
typically needs to be done is to specify the `--proxy` flag via the flod command
line or in the flod configuration file.  Typically the Tor proxy address will be
...
By default, flod assumes the proxy specified with `--proxy` is a Tor proxy and
...
discuss since flod will act as the server.
...
```text
HiddenServiceDir /var/tor/flod
HiddenServicePort 8333 127.0.0.1:8333
```

Once Tor is configured to provide the hidden service and you have obtained your
generated .onion address, configuring flod as a Tor hidden service requires
three flags:
...
```bash
$ ./flod --proxy=127.0.0.1:9050 --listen=127.0.0.1 --externalip=fooanon.onion
```
...
flod provides support for operating as a bridge between regular nodes and hidden
...
by using the `--onion` flag via the flod command line or in the flod
...
```bash
$ ./flod --onion=127.0.0.1:9050 --externalip=fooanon.onion
```
...
flod provides support for Tor stream isolation by using the `--torisolation`
...
```bash
$ ./flod --proxy=127.0.0.1:9050 --torisolation
```
